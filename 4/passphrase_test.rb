...
class PassphraseTest < MiniTest::Test
  def test_1
    assert Passphrase.new(%w(aa bb cc dd ee)).valid?
  end

  def test_2
    refute Passphrase.new(%w(aa bb cc dd aa)).valid?
  end

  def test_3
    assert Passphrase.new(%w(aa bb cc dd aaa)).valid?
  end
end
...
phrase_validities = passphrases.map do |passphrase|
  Passphrase.new(passphrase).valid?
end
