...
class Passphrase2
  def initialize(words)
    @words = words.map do |word|
      word.scan(/\w/).sort.join
    end
  end

  def valid?
    word_groups = @words.group_by { |word| word }
    word_groups.all? do |_, group|
      group.count == 1
    end
  end
end

class Passphrase2Test < MiniTest::Test
  def test_1
    assert Passphrase2.new(%w(abcde fghij)).valid?
  end

  def test_2
    refute Passphrase2.new(%w(abcde xyz ecdab)).valid?
  end

  def test_3
    assert Passphrase2.new(%w(a ab abc abd abf abj)).valid?
  end

  def test_4
    assert Passphrase2.new(%w(iiii oiii ooii oooi oooo)).valid?
  end

  def test_5
    refute Passphrase2.new(%w(oiii ioii iioi iiio)).valid?
  end
end
...
passphrases = words.split("\n").map { |s| s.split("\s") }
phrase_validities = passphrases.map do |passphrase|
  Passphrase2.new(passphrase).valid?
end
valid_passphrase_count = phrase_validities.select do |p| p end.count
puts "Valid passphrase count: #{valid_passphrase_count}"
